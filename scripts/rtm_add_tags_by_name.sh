...
# rtm_add_tags_by_name.sh — Add tags to the first incomplete task matching name:<term>
#
# Usage:
#   ./rtm_add_tags_by_name.sh "milk" "shopping,urgent"
...
if [[ -z "$TERM" || -z "$TAGS" ]]; then
  echo "Usage: $0 \"search term\" \"comma,separated,tags\"" >&2
  exit 2
fi
...
res=$(rtm_call "https://api.rememberthemilk.com/services/rest/?method=rtm.tasks.addTags&api_key=${RTM_API_KEY}&format=json&timeline=${TIMELINE}&list_id=${LIST_ID}&taskseries_id=${TASKSERIES_ID}&task_id=${TASK_ID}&tags=$(printf '%s' "$TAGS" | rtm_urlenc)&auth_token=${RTM_AUTH_TOKEN}&api_sig=${sig}")
rtm_fail_if_not_ok "$res"

echo "🏷️ Tags added: $NAME → $TAGS"
